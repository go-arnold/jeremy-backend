...
class Command(BaseCommand):
    ...
    def _upload_placeholder_images(self) -> dict:
        ...
        self.stdout.write(self.style.SUCCESS(f"Image pool ready: {uploaded} uploaded, {reused} reused."))
        return pool
    ...
    def _clear_data(self):
        ...
        from apps.artists.models import Artist, ArtistPhoto, ArtistVideo, Genre, Release
    ...
    def _seed_artists(self, count: int, genres, images: dict):
        ...
        for i, (name, city, genre_name) in enumerate(all_names[:count]):
            slug = (
                f"{name.lower().replace(' ', '-').replace("'", '')}-{i}"
                if i > 0
